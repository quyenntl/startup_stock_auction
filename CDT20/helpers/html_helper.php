	...
	function img($src = '', $index_page = FALSE)
	{
		...
		foreach ($src as $k=>$v)
		{
			...
			if ($k == 'src' AND strpos($v, '://') === FALSE)
			{
				...
				if ($index_page === TRUE)
				{
					$img .= ' src="'.$CDT->config->site_url($v).'"';
				}
				else
				{
					$img .= ' src="'.$CDT->config->slash_item('base_url').$v.'"';
				}
			}
			else
			{
...
if ( ! function_exists('link_tag'))
{
	function link_tag($href = '', $rel = 'stylesheet', $type = 'text/css', $title = '', $media = '', $index_page = FALSE)
	{
		...
		if (is_array($href))
		{
			foreach ($href as $k=>$v)
			{
				if ($k == 'href' AND strpos($v, '://') === FALSE)
				{
					if ($index_page === TRUE)
					{
						$link .= 'href="'.$CDT->config->site_url($v).'" ';
					}
					else
					{
						$link .= 'href="'.$CDT->config->slash_item('base_url').$v.'" ';
					}
				}
				else
				{
		...
		else
		{
			...
			{
				$link .= 'href="'.$CDT->config->site_url($href).'" ';
			}
			else
			{
				$link .= 'href="'.$CDT->config->slash_item('base_url').$href.'" ';
			}
