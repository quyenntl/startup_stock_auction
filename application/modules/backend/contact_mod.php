...
class contact_mod extends Module{
    function __construct(){
        ...
        if($cmd == 'del'){
            ...
            if($id){
                $this->gk_model->delete(FE_CONTACT,array('id'=>$id));                
            }
    ...
    function draw($params = array()){
        ...
        // }
        $total_data            = $this->gk_model->count(FE_CONTACT, $condition, $like);        
        ...
        $list_tmp  = $this->gk_model->select(FE_CONTACT, '*', $condition, $like, 'id DESC', $config['per_page'], $offset);
        ...
        $this->load->skins('backend');
        return $this->smarty->display_module('about/contact.html');
    }
}
?>