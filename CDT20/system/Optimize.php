...
class Optimize {
	...
	*/
	public function __construct()
	{
		...
		log_message('debug', 'optimize: Library initialized.');
		
		if( $this->CDT->config->load('optimize', TRUE, TRUE) ){
		
			log_message('debug', 'Optimize: config loaded from config file.');
			
			$optimize_config = $this->CDT->config->item('optimize');
			$this->config($optimize_config);
		}
	...
	*/	
	...
	{	
		...
		if($this->base_uri == '') $this->base_uri = $this->CDT->config->item('base_url');
		// use the provided values to define the rest of them
		$this->script_path = APP_PATH.$this->script_dir;
		$this->script_uri = $this->base_uri.$this->script_dir;
		
		$this->style_path = APP_PATH.$this->style_dir;
		$this->style_uri = $this->base_uri.$this->style_dir;

		$this->cache_path = APP_PATH.$this->cache_dir;
		$this->cache_uri = $this->base_uri.$this->cache_dir;

		log_message('debug', 'Optimize: library configured.');
	}
...
//					
				//	$this->_asset('js', $d, $p, $c, $m, NULL, $g);
                    $this->_asset('js', $file, $prod_file, $combine, $minify, NULL, $group);
...
//					$this->_asset('css', $d, $p, $c, $y, $m, $g);
                    $this->_asset('css', $file, $prod_file, $combine, $minify, $media, $group);
...
//			}
	...
	*/		
	public function group($group_name, $assets)
	{
		...
		if(!isset($assets['js']) && !isset($assets['css']) ){
			log_message('error', "Optimize: The asset group definition named '{$group_name}' does not contain a well formed array.");
			return;
		}
	...
	*/		
	...
	{	
		...
		switch($flag){
			...
			case 'both':
				$res =  $this->_display_js();
				$res .= $this->_display_css();
                return $res;
			break;
	...
	*/		
	private function _display_js($group = 'main')
	{
		...
		if( !isset($this->js[$group]) ): // the group you asked for doesn't exist. This should never happen, but better to be safe than sorry.
			...
			log_message('error', "Optimize: The JavaScript asset group named '{$group}' does not exist.");
			return;
		...
		// if we're combining files and minifying them
			...
			foreach($this->js[$group] as $ref):
				...
				if(!$ref['combine']):
					return (isset($ref['prod'])) ? $this->_tag('js', $ref['prod']) : $this->_tag('js', $ref['dev']);					
				elseif(!$ref['minify']):
			...
			return $this->_tag('js', $filename, TRUE);
	...
	*/		
	private function _display_css($group = 'main')
	{
		...
		if( !isset($this->css[$group]) ): // the group you asked for doesn't exist. This should never happen, but better to be safe than sorry.
			...
			log_message('error', "Optimize: The CSS asset group named '{$group}' does not exist.");
			return;
		...
		if($this->dev){ // we're in a development environment
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					return $this->_tag('css', $ref['dev'], FALSE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach ($refs as $ref):
					...
					if(!$ref['combine']):
						return (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], $media) : $this->_tag('css', $ref['dev'], $media);
					elseif(!$ref['minify']):
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'], 'minify'=>$ref['minify'] ) : array('dev'=>$ref['dev'], 'minify'=>$ref['minify']);
					else:
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'] ) : array('dev'=>$ref['dev']);
				...
				return $this->_tag('css',  $filename, TRUE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					return $this->_tag('css', $f, TRUE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					$f = (isset($ref['prod'])) ? $ref['prod'] : $ref['dev'];
					$res .= $this->_tag('css', $f, FALSE, $media);
	...
	private function _minify($flag, $file_ref)
	{
		...
		switch($flag){
			...
			case 'js':
				...
				return $this->CDT->jsmin->minify($contents);
			...
			case 'css':
				...
				$this->CDT->cssmin->config(array('relativePath'=>$rel));
                ...
                $contents    = preg_replace('/
                                            ...
                                            (["|\']?)
                                            \\)         # )
                                        /x', 'url($2?version='.$this->style_version.')', $contents);
                
                $start_pos   = strpos($ref,"webskins"); 
                $length      = strpos($ref,'styles') - $start_pos;
                $path        = str_replace("\\",DS, substr($ref,$start_pos,$length));
                $contents    = str_replace('../../../',$this->static_domain.$path,$contents);
                $contents    = str_replace('../../',$this->static_domain.$path,$contents);
    			$contents    = str_replace('../',$this->static_domain.$path,$contents);
                
				return $this->CDT->cssmin->minify($contents);
	...
	*/
	private function _get_contents($ref)
	{
		...
		if( $this->isURL($ref) && ( ini_get('allow_url_fopen') == 0 || $this->force_curl ) ):
			...
			$this->_load('curl');
			$contents = $this->CDT->curl->simple_get($ref);			
		else:
	...
	*/
	private function _cache($filename, $file_data)
	{
		...
		if(empty($file_data)):
			log_message('debug', 'Optimize: Cache file '.$filename.' was empty and therefore not written to disk at '.$this->cache_path);
			return false;
		endif;
		...
		if($success) : 
			log_message('debug', 'Optimize: Cache file '.$filename.' was written to '.$this->cache_path);
			return TRUE;
		else : 
			log_message('error', 'Optimize: There was an error writing cache file '.$filename.' to '.$this->cache_path);
			return FALSE;
		endif;
	}
	...
	{   
		switch($flag){
			...
			case 'js':
				...
				return '<script type="text/javascript" src="'.$dir.$ref.'" charset="'.$this->CDT->config->item('charset').'"></script>'."\r\n";
			...
			case 'css':
				...
				foreach( $files as $file ){
					...
					if ( ($ext == $flag) && $fl >= 42 && ( filemtime( $this->cache_path . $file ) < $before) ) {
						...
						if($success) : log_message('debug', 'Optimize: Cache file '.$file.' was removed from '.$this->cache_path);
						else : log_message('error', 'Optimize: There was an error removing cache file '.$file.' from '.$this->cache_path);
						endif;
	...
	*/
	private function _load($lib=NULL)
	{
		...
		if( isset($this->loaded[$lib]) ):
			...
			$this->CDT->load->library($lib);
			$this->loaded[$lib] = TRUE;

			log_message('debug', 'Optimize: CDT library '."'$lib'".' loaded');
			return TRUE;
		endif;
	}
