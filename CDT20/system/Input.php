...
class Input {
	...
	*/
	 ...
	 *         
	 **/
    public function array_to_option($options,$selected = 0){
        $input = '';
    ...
    public function array_to_multiple_checked($options,$arr_selected=array()){
        $input = '';
        if (is_array($options) and sizeof($options) > 0)
			foreach ( $options as $key => $text ) {
				$input .= '<option value="' . $key . '"';
    ...
    public function remove_external_link($text)
	{
		//$text=str_replace('href',' rel="nofollow" href',$text);
		//return $text;
		// Optimize để chỉ đặt với domain ngoài 1TOP
		$regex = '#(href=").*?(")#';
