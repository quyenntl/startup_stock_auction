...
$.extend(History.prototype, {
	...
	init: function(callback) {
		...
		this._callback(this._curHash.replace(/^\/edit\//, ''));
	...
	_check: function() {
		if($.browser.msie) {
			...
			if(current_hash != $.history._curHash) {
				...
				$.history._callback(current_hash.replace(/^\/edit\//, ''));
			...
			if (!$.history._dontCheck) {
				...
				if (historyDelta) { // back or forward button has been pushed
					...
					if (location.hash.indexOf('/edit/') >= 0) {
						$.history._callback(location.hash.split('/edit/')[1]);
					} else {
						$.history._callback('');
					}
	...
	load: function(hash) {
		...
		if ($.browser.safari) {
			...
			newhash = (this.isKonqueror() ? '' : '/edit/') + hash;
