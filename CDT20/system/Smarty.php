...
class Smarty extends Smarty_Internal_Data {
	...
	*/
    ...
    { 
        ...
        // set default dirs
        $this->template_dir = array(APP_PATH . 'webskins' . DS.'templates'.DS);
        $this->compile_dir = APP_PATH.'webskins'.DS.'templates_c' . DS;
        $this->plugins_dir = array(SMARTY_PLUGINS_DIR);
        $this->cache_dir = APP_PATH . 'cache' . DS . 'template'. DS;
        $this->config_dir = APP_PATH . 'configs' . DS;
    ...
    /**
     * assigns a Module - Author: KienNT
     * 
     * @param array $ |string $tpl_var - Ten gia tri goi trong Temp
     * @param mixed $value gia tri khai bao
     * @param array $ |string $config - Khai bao cac gia tri truyen vao Module
     * @param boolean $nocache if true any output of this variable will be not cached
     * @param boolean $scope the scope the variable will have  (local,parent or root)
     */
    public function assign_module($tpl_var='', $mod_name='', $config=array(), $nocache = false)
    {
        if (is_array($tpl_var)) {
            foreach ($tpl_var as $_key => $_val) {
                    if (is_array($_val)) {
                        $this->assign($_key,$this->CDT->load->load_module($_val[0],$_val[1]),$nocache);
                        }
                    else{
                        $this->assign($_key,$this->CDT->load->load_module($_val),$nocache);
                    }
            } 
        } else {
            if ($tpl_var != '') {
                $this->assign($tpl_var,$this->CDT->load->load_module($mod_name,$config),$nocache);
            } 
        } 
    }
    /**
     ...
     */
    public function display($template, $cache_id = null, $compile_id = null, $parent = null,$display=true)
    { 
        // display template
        $this->smarty->template_dir = LAYOUT_PATH.DS;
        $this->smarty->compile_dir  = TEMPLATE_PATH_COMPILE.SKINS_TEMPLATE.DS;
        $this->smarty->cache_dir    = CACHE_PATH.SKINS_TEMPLATE.DS; 
        // xu ly write js va css
        //$this->CDT->load->cache(array("adapter"=>"file","cache_path"=>CACHE_PATH.'cache_script'.DS));
  
        if(is_array($this->CDT->static_module_script) && is_array($this->CDT->static_script)){
            
            //$file_cache_js = md5(implode('&@&',$this->CDT->static_module_script));
            //$combie = $this->CDT->cache->get($file_cache_js.".script");
            $header_js = array();
            $footer_js = array();
            $array_css = array();
            
            if($combie == false){
                foreach($this->CDT->static_script as $value){
                    if(is_array($value['js']['header'])){
                        foreach($value['js']['header'] as $js_hvalue){
                            $header_js[] = $js_hvalue;
                        }
                    }
                    
                    if(is_array($value['js']['footer'])){
                        foreach($value['js']['footer'] as $js_fvalue){
                            $footer_js[] = $js_fvalue;
                        }
                    }
                    
                    if(is_array($value['css'])){
                        foreach($value['css'] as $css_value){
                            $array_css[] = $css_value;
                        }
                    }
                ...
                $combie = array(
                    'header_js' => array_unique(array_merge($header_js)),
                    'footer_js' => array_unique(array_merge($footer_js)),
                    'css'       => array_unique($array_css)
                );
                //$this->CDT->cache->save($file_cache_js.".script",$combie,86400);
            }
        }
        
        // Combie Header JS & Css
        $this->CDT->optimize->group('header_script', array('css'=>$combie['css'],'js'=>$combie['header_js'])); 
        $this->smarty->assign('header_script',$this->CDT->optimize->display('header_script')); 
        // Combie Footer JS
        $this->CDT->optimize->group('footer_script', array('js'=>$combie['footer_js'])); 
        $this->smarty->assign('footer_script',$this->CDT->optimize->display('footer_script'));    
        
        return $this->fetch ($template, $cache_id, $compile_id, $parent, $display);
    } 
    public function display_module($template, $cache_id = null, $compile_id = null, $parent = null,$display=false)
    { 
        // display template
        return $this->fetch ($template, $cache_id, $compile_id, $parent, $display);
    } 
