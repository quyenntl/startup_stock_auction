...
class add_pro_mod extends Module{
    public $notice = '';
    public $type   = 0; 
    ...
    function submit(){        
        ...
        $data['link_ytb']          = $this->input->post('link_video');
        $data['category_id']        = $this->input->post('guide_category_id');
        ...
        if($imageName){
            ...
            $config['upload_path']      = STORE_DATA.'products';
        ...
        if(!$id)
        {
            $this->fe_model->insert(FE_PRODUCTS, $data);
            $this->notice = 'Thêm mới thành công';
            redirect('admin/products');
        }
        else if($id > 0)
        {
            $this->fe_model->update(FE_PRODUCTS, array('id' => $id), $data);
    ...
    function draw($params){       
        ...
        {   
            ...
            $data      = $this->fe_model->select(FE_PRODUCTS, '*', $condition, $like, 'time_created DESC', $config['per_page'], $offset);
            $user_list = array();
            $cat_list  = array();
        ...
        if($params['mode'] == 'form')
        {
            ...
            if($id)
            {
                $data = $this->fe_model->select_one(FE_PRODUCTS, '*', array('id' => $id));
                $page_title = 'Sửa sản phẩm - '.$data['title'].' | Quản trị';
            }
            else
            {
                $page_title = 'Thêm sản phẩm | Quản trị';
            }            
            ...
            $assign = array(
                ...
                'category_list' => $category_list,
                'form_edit'     => form_open('backend/add_pro_mod', '',array('enctype'=>'multipart/form-data', 'id' => 'form_guide_edit')),
                'notice'        => $this->notice,
                'data'          => $data
            );            
            
            $this->smarty->assign($assign);
            return $this->smarty->display_module('products/form.html');
        }
    }
}
